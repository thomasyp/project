...
with open(resultfile, 'w') as fid, open(seachoutfile, 'w') as fid2:
   fid.write("{:^10} {:^10} {:^10} {:^10} {:^20} {:^20} {:^20} {:^20}\n"\
       .format('Nacl', 'PuCl3', 'ThCl4', 'Keff',\
                              'CR of kcode', 'Escape of kcode', 'CR of fixed',\
                              'Escape of fixed'))
   fid2.write("{:^10} {:^10} {:^10} {:^10} {:^20} {:^20} {:^20} {:^20}\n"\
       .format('Nacl', 'PuCl3', 'ThCl4', 'Keff',\
                               'CR of kcode', 'Escape of kcode', 'CR of fixed',\
                               'Escape of fixed'))
...
for ii in np.arange(startmolnacl, endmolnacl, stepmolnacl):
    ...
    for jj in np.arange(100-ii, endmolpucl, stepmolpucl):
        ...
        if os.path.isfile(inp+'o'):
            ...
            if os.path.isfile(inp+'o'):
                oldfilename = inp+'o'
                newfilename = inp+'ko_'+'{:.2f}'.format(matdict[nacl])+'_'+'{:.2f}'.format(matdict[pucl3])+\
                    '_'+'{:.2f}'.format(matdict[thcl4])
        ...
        if results['keff'] < 0.998:
            ...
            if os.path.isfile(inp+'o'):
                oldfilename = inp+'o'
                newfilename = inp+'fo_'+'{:.2f}'.format(matdict[nacl])+'_'+'{:.2f}'.format(matdict[pucl3])+\
                    '_'+'{:.2f}'.format(matdict[thcl4])
                mh.deleteFiles(newfilename)
                os.rename(oldfilename, newfilename)
            mh.cleanup(inp)
        ...
        with open(resultfile, 'a') as fid, open(seachoutfile, 'a') as fid2:
            fid2.write("{nacl:^10.4f} {pucl3:^10.4f} {thcl4:^10.4f} {keff:^10} {kCR:^20.4f} {kescape:^20.4f} {fCR:^20.4f} {fescape:^20.4f}\n".format(**results))
            if float(results['keff']) > 0.97 and float(results['keff'])<0.99:
                fid.write("{nacl:^10.4f} {pucl3:^10.4f} {thcl4:^10.4f} {keff:^10} {kCR:^20.4f} {kescape:^20.4f} {fCR:^20.4f} {fescape:^20.4f}\n".format(**results))
