...
def readinp(inp):
    ...
    with open(inp, 'r') as fid:
        for line in fid:
            ...
            if bool(lists):
                if re.match('thicknessStep', lists[0], re.I) is not None:
                    thicknessStep = lists[1:]
                if re.match('startmolnacl', lists[0], re.I) is not None:
                    intime = lists[1:]
                if re.match('startmolpucl', lists[0], re.I) is not None:
                    intime = lists[1:]
                if re.match('stepmolnacl', lists[0], re.I) is not None:
                    intime = lists[1:]
                if re.match('stepmolpucl', lists[0], re.I) is not None:
                    stepmolpucl = lists[1:]
                if re.match('stepmolnacl', lists[0], re.I) is not None:
                    intime = lists[1:]
    return startmolnacl, startmolpucl, stepmolnacl, stepmolpucl
